...
// TestNPCMemoryAppendsAfterLoop verifies that after a say → reply
// round-trip via the per-NPC loop, the NPC's room-observations bucket
// (playerID=0) holds both turns: the observed utterance and the NPC's
// own reply. The loop deliberately stores all observations under
// playerID=0 — see loop/dispatch.go.
func TestNPCMemoryAppendsAfterLoop(t *testing.T) {
	...
	// Give the loop a moment to record its own reply after the
	// broadcast lands.
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		if len(n.Memory.Recent(0, 10)) >= 2 {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}

	turns := n.Memory.Recent(0, 10)
	if len(turns) < 2 {
		t.Fatalf("Recent length = %d, want >=2 (observation + npc reply); got %+v", len(turns), turns)
	}
	if !strings.Contains(turns[0].Text, "bartender") {
		t.Errorf("observation turn text = %q, want one containing 'bartender'", turns[0].Text)
	}
	// The last turn should be the NPC's own reply.
	last := turns[len(turns)-1]
	if last.Speaker != "<npc>" {
		t.Errorf("last turn speaker = %q, want <npc>", last.Speaker)
	}
	if !strings.Contains(last.Text, "bartender") {
		t.Errorf("NPC turn text = %q, want one mentioning 'bartender'", last.Text)
	}
}
...
func TestNPCMemoryPersistsAcrossRestart(t *testing.T) {
	...
	// --- First boot: engage via the loop, then shutdown drain. ---
	...
	bus1 := events.NewMemBus()
	w1.SetBus(bus1)
	a1 := auth.NewStore(db1)
	reg1, err := Load(context.Background(), db1, w1, fakeDefaults(), logger, LoopDeps{
		Bus:    bus1,
		World:  loopBroadcaster{w: w1},
		Config: LoopConfig{Debounce: 30 * time.Millisecond},
	})
	if err != nil {
		t.Fatalf("first npc.Load: %v", err)
	}
	w1.SetSayObserver(func(roomID world.RoomID, speakerID world.ObjectID, speakerName, text string) {
		reg1.HandleSay(roomID, speakerID, speakerName, text)
	})
	alice1 := attachPlayerOn(t, w1, a1, "alice")
	alice1.drain()

	if err := w1.Say(alice1.Presence, "hi bartender"); err != nil {
		t.Fatalf("Say: %v", err)
	}
	// Wait for the loop to broadcast the reply.
	alice1.waitFor(t, bartenderResponse, 3*time.Second)
	...
	bus2 := events.NewMemBus()
	w2.SetBus(bus2)
	reg2, err := Load(context.Background(), db2, w2, fakeDefaults(), logger, LoopDeps{
		Bus:    bus2,
		World:  loopBroadcaster{w: w2},
		Config: LoopConfig{Debounce: 30 * time.Millisecond},
	})
	if err != nil {
...
// or before its context's deadline even when in-flight observer work
// would otherwise run longer.
func TestNPCRegistryShutdownHonoursContext(t *testing.T) {
	e := newFakeBackendEnv(t)

	// Pin a fake in-flight observer goroutine by adding to inFlightWG
	// directly and never calling Done. Shutdown must not wait for it
	...
	// Allow generous slack so a slow CI runner doesn't flake.
...
func TestNPCMemoryReloadDrainsOldStates(t *testing.T) {
	...
	// Drive a conversation through the bus so the bartender's short-term
	// buffer has content that MUST be summarised before the old workers
	// are torn down.
	if err := e.world.Say(alice.Presence, "hi bartender"); err != nil {
		t.Fatalf("Say: %v", err)
	}
	alice.waitFor(t, bartenderResponse, 3*time.Second)
	// Brief delay so the loop records its own reply.
	time.Sleep(100 * time.Millisecond)

	// Sanity: no memory row yet.
	var rows int
...
// TestNPCMemoryEndsOnDetach verifies that when a player detaches —
...
func TestNPCMemoryEndsOnDetach(t *testing.T) {
	for _, reason := range []world.DisconnectReason{world.DisconnectQuit, world.DisconnectDropped} {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Seed alice's per-player bucket so onPlayerDetach has
			// something to drain. The M7 loop stores observations in
			// bucket 0; per-player conversation ergonomics are still
			// covered by the M3-era EndConversation path the registry
			// invokes from its detach/move observers.
			n.Memory.Append(alice.PlayerID, memory.Turn{Speaker: "alice", Text: "hi bartender"})
			n.Memory.Append(alice.PlayerID, memory.Turn{Speaker: "<npc>", Text: bartenderResponse})
...
func TestNPCMemoryEndsOnMoveAway(t *testing.T) {
	...
	// Seed alice's per-player bucket — see TestNPCMemoryEndsOnDetach.
	n.Memory.Append(alice.PlayerID, memory.Turn{Speaker: "alice", Text: "hi bartender"})
	n.Memory.Append(alice.PlayerID, memory.Turn{Speaker: "<npc>", Text: bartenderResponse})
...
// memory matching the conversation topic appears when retrieved
// directly (we don't observe through the LLM context here since the
// loop's per-NPC retrieval happens deep inside Tick).
func TestNPCMemoryRetrievesPriorMemories(t *testing.T) {
	...
	// Prime short-term so Retrieve has something to embed.
	n.Memory.Append(alice.PlayerID, memory.Turn{Speaker: "alice", Text: "hey bartender, the gate again"})
